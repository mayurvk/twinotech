...
exports.compileCPP = function ( envData ,  code , filename, fn ) { 
					    ...
					    console.log('INFO: '.green + filename +'.cpp created');	
	 					//compiling and exrcuiting source code
					       if(envData.cmd === 'g++')
					       {
							...
							//compile c code
                           //edited by pg
							//commmand = 'g++ ' + path + filename +' -o '+path + filename +'.exe' 
...
exports.compileCPPWithInput = function ( envData , code , input ,filename,   fn ) { 
	//var filename = cuid.slug();
	path = './temp/';
						...
						exec(commmand , function ( error , stdout , stderr ){  
							...
							else
							{
								if(input){
						            ...
						            var tempcommand = "./temp/" + filename ;
									///var/www/html/twinotech/temp/ is absolute path of input file
									exec( tempcommand + '<' + '/var/www/html/twinotech/temp/'+inputfile , function( error , stdout , stderr ){
									if(error)
									{
									if(error.toString().indexOf('Error: stdout maxBuffer exceeded.') != -1)
										{
